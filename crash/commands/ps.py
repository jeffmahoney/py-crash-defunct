...
from crash.types import list
import crash.cache.tasks
import crash.cache.table

from crash.cache.tasks import cache as task_cache
...
# 17077  16749   0  ffff8800b956b848 RU   0.0      0      0  [less]
    ...
    def task_header(self, task):
        ...
        return template.format(task.pid, task.address(), task.cpu(),
                               task.comm)
    ...
    def print_last_run(self, task):
        ...
        print template.format(task.last_run(), radix_string,
                              task.task_state_string(), self.task_header(task))

    def print_one(self, argv, task):
        specified = argv.args == None
        task.update()

        pointer = task.task_struct.address
        if argv.s:
            pointer = task.task_struct['stack'].address
        ...
        try:
            parent = task_cache.task_by_addr(task.parent)
        ...
        print self.line_template.format(" ", task.pid, parent_pid,
                                        task.thread_info['cpu'],
                                        long(pointer), 16,
                                        task.task_state_string(),
                                        task.pct_physmem, 
    ...
    def execute(self, argv):
        sort_by_pid = lambda x: x.pid
        sort_by_last_run = lambda x: -x.last_run()
        ...
        if not argv.args:
            for task in task_cache.tasks(sort_key=sort_by):
                if task:
